...
class ScriptLoader {
     ...
     * @param {string} scriptPath - Path to the script file relative to panels/components/scripts/
    ...
    async loadScript(scriptPath) {
        ...
        try {
            ...
            let fullPath;
            try {
                fullPath = chrome.runtime.getURL(`panels/components/scripts/${scriptPath}`);
            } catch (e) {
                    ...
                    xhr.onreadystatechange = function() {
                        if (xhr.readyState === 4) {
                            if (xhr.status === 200) {
                                ...
                                reject(new Error(`Failed to load script: ${xhr.statusText} (${xhr.status})`));
                            }
                        }
                    };
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    chrome.devtools.inspectedWindow.eval(wrappedScript, (result, isException) => {
                        if (isException) {
                            const errorMessage = isException.value || isException.description || JSON.stringify(isException);
